...
import { useLoginMutation, useRegisterMutation } from "./store/slices/authApiSlice";
...
const RegisterPage: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    const userRegisterData = await register({ name: data.Name, email: data.Email, password: data.Password, role: 'USER' }).unwrap();
    const userLoginData = await login({ email: userRegisterData.email, password: data.Password }).unwrap()
