...
import { calculateCeilTotalPaginateItem } from "lib/utils/helper";
...
const Paginate: React.FC<PaginateProps> = ({
    ...
    return (
      <div>
        {[...Array(total)].map((_, idx) => {
          ...
          return (
            <button
              className={`border border-primary p-2  ${page === pageNumber ? "bg-primary text-white" : ""
                }`}
              key={pageNumber}
  ...
  return (
    ...
    >
      ...
      <button
        className={`border border-primary p-1 ${page === ceilTotalItem && "bg-green-200"
          }`}
